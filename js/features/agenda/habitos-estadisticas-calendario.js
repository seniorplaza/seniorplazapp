...
function _renderEditHabitoFrecDetalle() {
    ...
    if (frec === 'cada_x_dias') {
        ...
        wrap.style.display = 'block';
        detalle.innerHTML = `<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;"><input type="number" id="editHabitoVecesPeriodo" min="1" max="30" value="${window._editHabitoVecesPeriodo || 3}" oninput="window._editHabitoVecesPeriodo=Math.max(1, +this.value || 1)" style="width:64px;background:#1e293b;border:1px solid rgba(59,130,246,0.3);border-radius:10px;color:#f1f5f9;font-size:15px;font-weight:700;padding:10px;outline:none;text-align:center;"><span style="color:#94a3b8;font-size:13px;">veces por</span><div class="agenda-inline-select-wrap"><select id="editHabitoVecesPeriodoPer" class="agenda-inline-select" onchange="window._editHabitoVecesPeriodoPer=this.value"><option value="semana" ${(window._editHabitoVecesPeriodoPer||'semana')==='semana'?'selected':''}>semana</option><option value="mes" ${(window._editHabitoVecesPeriodoPer||'semana')==='mes'?'selected':''}>mes</option><option value="anio" ${(window._editHabitoVecesPeriodoPer||'semana')==='anio'?'selected':''}>año</option></select><span class="material-symbols-rounded agenda-inline-select-icon">expand_more</span></div></div>`;
    } else {
