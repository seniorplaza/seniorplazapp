                ...
                (function() {
                    document.addEventListener('DOMContentLoaded', function() {
                        var wrapper = document.getElementById('est-intervalo-wrapper');
                        if (!wrapper) return;
                        ...
                        wrapper.addEventListener('touchend', function(e) {
                            ...
                            if (typeof estNavIntervalo === 'function') {
                                estNavIntervalo(dx > 0 ? -1 : 1);
                                if (navigator.vibrate) navigator.vibrate(12);
                            }
                        }, { passive: true });
                        (function() {
                            var el = document.getElementById('est-intervalo-label');
                            if (el && (el.textContent === 'Este mes' || el.textContent.trim() === 'Este mes')) {
                                var n = new Date();
                                var m = n.toLocaleDateString('es-ES', { month:'long', year:'numeric' });
