    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const tabOrder = [
            ...
            'fitness-gimnasio', 'fitness-nutricion', 'fitness-progreso'
        ];
        ...
        (function() {
            ...
            function swIsBlocked(target) {
                const tag = target.tagName ? target.tagName.toLowerCase() : '';
                if (tag === 'input' || tag === 'textarea' || tag === 'select') return true;
                if (target.closest && (
                    target.closest('#op-intervalo-wrapper') ||
                    target.closest('#est-intervalo-wrapper') ||
                    target.closest('#diario-intervalo-wrapper') ||
                    target.closest('#tareas-intervalo-wrapper') ||
                    target.closest('#gym-intervalo-wrapper') ||
                    target.closest('#nutri-intervalo-wrapper')
                )) return true;
                if (target.closest && (
                    target.closest('#cal-grid') ||
                    target.closest('#eop-calendar-panel') ||
                    target.closest('.cal-grid') ||
                    target.closest('.calendario') ||
                    target.closest('[data-calendar]') ||
                    target.closest('[ontouchstart*="_calSwipe"]')
                )) return true;
                const isVisor = target.closest && (
                    target.closest('#visor360-container') ||
                    target.closest('#visor-main-container') ||
                    target.closest('#canvas-container-360') ||
                    tag === 'canvas'
                );
                if (isVisor) {
                ...
                if (target.closest && (
                    target.closest('.carrusel-scroll-wrapper') ||
                    target.closest('.carrusel-track')
                )) return true;
                return false;
            }
            ...
            document.addEventListener('touchstart', function(e) {
                ...
                swTracking = !swIsBlocked(e.target);
                if (!swTracking) return;
                swTouchStartX = t.screenX;
                swTouchStartY = t.screenY;
                swStartTime = Date.now();
            }, { passive: true, capture: true });

            document.addEventListener('touchend', function(e) {
                if (!isMobile() || !swTracking) { swTracking = false; return; }
                swTracking = false;
                if (swIsBlocked(e.target)) return;

                const t = e.changedTouches[0];
                const dx = t.screenX - swTouchStartX;
                const dy = t.screenY - swTouchStartY;
                const dt = Date.now() - swStartTime;
                if (Math.abs(dx) < Math.abs(dy) * 1.2) return;
                const minDist = dt < 300 ? 45 : 90;
                if (Math.abs(dx) < minDist) return;

                const activeTab = document.querySelector('.tab-content.active');
                if (!activeTab) return;
                const currentId = activeTab.id;
                if (currentId === 'fitness-gimnasio') return;
